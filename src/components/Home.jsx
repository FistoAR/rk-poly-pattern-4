...
// ********Sweet Box Tampmer Evident 250 Pattern**********
sb_te_250_1_1,
sb_te_250_2_1,
sb_te_250_1_2,
sb_te_250_2_2,
...
const categories = [
  ...
  },
  {
  ...
  },
   {
    ...
    title: "Sweet Box",
    image: sweetBoxImg,
    sections: [ // ← NEW: Using "sections" instead of flat "items"
      {
        id: "plain",
        heading: "Plain", // Top IML only
        items: [
          {
            id: "250ml_plain",
            label: "250 gms ",
...
{
  id: "4side",
  heading: "4 Side IML", // Top + 4 Sides
  items: [
    {
      id: "250g_4side",
      label: "250 gms",
      display: ` **250 gms** Sweet Box`,
      path: sb_4_250,
      image: sb250gImage,
    ...
    },
    {
      ...
      {
        id: "5side",
        heading: "5 Side IML", // Top + Bottom + 4 Sides
        items: [
          {
            id: "250g_5side",
            label: "250 gms",
    ...
    items: [
      // ... existing items (now with 1000gms added)
      {
        id: "250_tr",
        label: "250 gms",
        display: `**250 gms** Tamper Evident`,
        path: te_250,
        image: sbte250gImage,
        dimensions_lid: "1512px X 1152px (W x H)",
        dimensions_tub: "2402px X 2048px (W x H)",
      ...
      },
      {
        id: "500_tr",
        label: "500 gms",
        display: `**500 gms** Tamper Evident`,
        path: te_500,
        image: sbte500gImage,
        dimensions_lid: "1630px X 1288px (W x H)",
        dimensions_tub: "2398px X 2045px (W x H)",
...
const setPatternModel = async (patternImage) => {
  ...
  const labelMat = materials.find((m) => m.name === "texture_area");  // ✅ FIXED
  ...
  const underMat = materials.find((m) => m.name === "texture_under");  // ✅ FIXED
  if (underMat) {
...
const setPatternModelSB5 = async (patternImage1, patternImage2) => {
  ...
  setViewMode("iml"); // ✅ Ensure IML mode is active
  ...
  if (!labelMat) {
    console.warn("⚠ No 'texture_area' material found for lid pattern");
    console.log("Available materials:", materials.map((m) => m.name));
    return;
  }

  console.log("✓ Applying lid pattern to 'texture_area':", patternImage1);
  ...
  if (!underMat) {
    console.warn("⚠ No 'texture_area2' material found for tub pattern");
    return;
  }

  console.log("✓ Applying tub pattern to 'texture_area2':", patternImage2);
  ...
  mv.requestUpdate();
  console.log("✓ Both lid and tub patterns applied successfully");
};
...
useEffect(() => {
  ...
  const handleLoad = () => {
    ...
    const materials = mv.model?.materials;
    if (materials) {
      console.log("🔍 Available materials for", selectedModel, ":", materials.map((m) => m.name));
...
const applyTextureToMaterial = async ({ event, materialName, modelRef, modelLoaded }) => {
  const file = event.target.files?.[0];
  if (!file || !modelRef.current || !modelLoaded) {
    console.warn("⚠ Upload conditions not met:", { file, modelRef: !!modelRef.current, modelLoaded });
    return;
  }
  ...
  reader.onload = async (e) => {
    ...
    console.log("🔍 All available materials:", materials?.map((m) => m.name));
...
const getSelectedItem = () => {
  for (const cat of categories) {
    ...
    if (cat.sections) {
      for (const section of cat.sections) {
        const found = section.items.find((item) => item.path === selectedModel);
...
useEffect(() => {
  ...
  } else if (categoryId === "sweet-box-all") { // ← UPDATED
    setTopColor("#ffffff");
    setBottomColor("#ffffff");
...
const sendImageToServer = async (file, userData, modelPath) => {
  ...
  try {
    ...
    await fetch('https://www.fist-o.com/rk_pattern/file_upload.php', {
      method: 'POST',
      body: formData
    });
...
>
  <img src={upArrowUploadIcon} className="w-[2vw] h-[2.75vh]" />
  <div>UPLOAD LID</div>
</button>
...
>
  <img src={upArrowUploadIcon} className="w-[2vw] h-[2.75vh]" />
  <div>{getModelCategory() === "sweet-box-all" && selectedItem?.id?.includes("4side") ? "UPLOAD 4 SIDE" : "UPLOAD TUB"}</div>
</button>
      ...
      {selectedItem && (
        <>
          {needsDualUpload() ? (
            ...
            <>
              {selectedItem.lidPatterns?.map((lidPattern, index) => {
                const tubPattern = selectedItem.tubPatterns[index];
                const isSelected = viewMode === "iml" && selectedLidPattern === index + 1;
            ...
            <>
              {selectedItem.patterns?.map((pattern, index) => {
                const isSelected = viewMode === "iml" && selectedPattern === index + 1;
  ...
  <div className="relative">
    <input
      ...
      onChange={(e) => {
        ...
        if (/^#[0-9A-F]{0,6}$/.test(value) || value === '' || value === '#') {
          if (activePart === "lid") {
            setTopColor(value);
            setLidColorMode("custom");
          } else {
    ...
    />
        ...
        <div className=" w-[25vw] bg-[#2F5755] shadow-lg p-[1.5vw] pt-[4vw]  overflow-y-hidden space-y-[1vw]   ">
          ...
          <div className="">
           ...
           <div className="overflow-y-auto max-h-[65vh] pr-[1vw]" style={{ paddingBottom: "1vw" }}>
  {categories.map((cat) => {
      ...
      >
        ...
        >
          <div className="flex items-center gap-[1vw] ">
            <img src={cat.image} className="w-[4vw]" alt={cat.title} />
              ...
              {cat.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span
                      className={`text-[.65vw] mt-[1.5vw] w-full ${isSelected
                          ? "text-white "
                          : "text-black"}`}
                      dangerouslySetInnerHTML={{ __html: item.display }}
                    />
